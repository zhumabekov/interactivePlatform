<template>
  <section>
    <div class="wrapper" v-if="loaded">
      <router-link
        :to="
          lection.lesson.type === 3
            ? '/infoblock/' + lection.id
            : lection.lesson.type === 2
            ? '/infocart/' + lection.id
            : '/lectureblock/' + lection.id
        "
        class="inform_block"
        v-for="(lection, index) of lections"
        :key="index"
      >
        <div
          class="inform_block__img"
          :style="{
            'background-image': `url(http://176.119.159.61/fileitems/${lection.lesson.image.id}${lection.lesson.image.extension})`,
          }"
        ></div>
        <div class="inform_block__titile">
          {{ index + 1 }}. {{ lection.lesson.name }}
        </div>
        <div class="inform_block__text">
          <span>{{ lection.lesson.description }}</span>
        </div>
        <div class="inform_block__progress_bar">
          <div
            class="percentage"
            :class="{
              stopAnimation: lection.passed * (100 / lection.total) == 100,
            }"
            :style="{ width: lection.passed * (100 / lection.total) + '%' }"
          >
            {{ (lection.passed * (100 / lection.total)).toFixed() }} %
          </div>
        </div>
      </router-link>
    </div>
