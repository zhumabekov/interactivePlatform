<template>
  <section>
      <div v-if="(lectionData.passed === lectionData.total) && loaded" class="callback">
        <img src="../assets/certificate.png"/>  Тест сдан!
        <router-link tag="button" to="/lection" class="callback_btn">Назад</router-link>
      </div>
      <article class="info_cart" v-else-if="loaded">
        <div class="inform_cart__content" @click="toogleQuestionAnswer()" :class="{leaveBlockLeft: leaveLeft, leaveBlockRight: leaveRight}">
          <div class="front" :class="{activeAnswer: isActive}"><span>{{activeCart.situation}}</span></div>
          <div class="back" :class="{inactiveQuestion: isActive}"><span>{{activeCart.riskLevel}}</span></div>
        </div>
